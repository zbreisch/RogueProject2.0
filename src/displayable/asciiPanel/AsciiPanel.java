package src.displayable.asciiPanel;
...
public class AsciiPanel extends JPanel {
    ...
    public AsciiPanel(int width, int height, AsciiFont font) {
        ...
        if (DEBUG > 0) {
            System.out.println("width in Characters: "+width+", height in characters: "+height);
        }
    ...
    private void loadGlyphs() {
        try {
            if (DEBUG > 0) {
	       System.out.println("SPM: terminalFontFile: "+terminalFontFile);
	       System.out.flush( );
            }
    ...
    public AsciiPanel write(char character, int x, int y, Color foreground, Color background) {
        // SPM next line
        if (DEBUG > 0) {
           System.out.println("write(c,i,i,C,C), x"+x+", y: "+y+"{, character: "+character);
        }
        if (character < 0 || character >= glyphs.length)
            throw new IllegalArgumentException("character " + character + " must be within range [0," + glyphs.length + "]." );
